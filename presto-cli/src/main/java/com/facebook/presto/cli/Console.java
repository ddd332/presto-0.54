...
import com.facebook.presto.cli.sparql2sql.GetSQL;
        ...
        try (TableNameCompleter tableNameCompleter = new TableNameCompleter(clientOptions.toClientSession(), queryRunner);
            ...
            while (true) {
                ...
                for (Statement split : splitter.getCompleteStatements()) {
                    ...
                    String queryString = split.statement();
                    String language = session.getLanguage();
                    if(language!=null && (language.equals("SPARQL") || language.equals("sparql"))) {
                        // rewrite sparql query as SQL and print it.
                        queryString = (new GetSQL()).parse(queryString);
                        buffer.append(queryString).append("\n");
                    }
                    /**
                     * add code here. Search mysql to see the table. Then transfer SQL if possible.
                     */
                    
                    process(queryRunner, queryString, outputFormat, true);
                    reader.getHistory().add(squeezeStatement(queryString) + split.terminator());
                }
